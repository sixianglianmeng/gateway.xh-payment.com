...
//            self::payFail($newOrder,$failMsg);
            $newOrder->status = Order::STATUS_NONE;
            $newOrder->fail_msg .= $failMsg."\n";
            $newOrder->bak .= $failMsg."\n";
            $newOrder->save();
            throw new OperationFailureException($failMsg,$e->getCode());
        }
...
//            }
     ...
     * @param String $bak 备注
     */
    static public function frozen(Order &$order, $opUid, $opUsername, $bak='', $ip='',$track_type=0,$category=0){
        Yii::info(__FUNCTION__.' '.$order->order_no.' '.$bak);
...
//        $order->op_username = $opUsername;
    ...
    static public function unfrozen(Order &$order, $opUid, $opUsername, $bak='', $ip=''){
        ...
        $order->category = Order::CATEGORY_ZORE;
        $order->save();
